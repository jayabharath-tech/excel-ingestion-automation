...
class TestRepairSAIdUsingDOB:
    ...
    def test_repair_11_digit_id_born_2000(self):
        ...
        df = pd.DataFrame({
            "IdNo": ["01026128186"],  # 11 digits, missing "00"
            "Dob": [pd.to_datetime("02/01/2000")]  # Born 2000
        })
    ...
    def test_repair_12_digit_id_born_2001(self):
        ...
        df = pd.DataFrame({
            "IdNo": ["1026128186"],  # 12 digits, missing "0"
            "Dob": [pd.to_datetime("02/01/2001")]  # Born 2001
        })
        
        result_df = repair_sa_id_using_dob(df, "IdNo", "Dob")
        
        # Should be padded with "0" for 2001-2009 birth years
        assert result_df.loc[0, "IdNo"] == "01026128186"

    def test_repair_12_digit_id_born_2005(self):
        """Test repairing 12-digit ID for person born in 2005."""
        df = pd.DataFrame({
            "IdNo": ["5026128186"],  # 12 digits, missing "0"
            "Dob": [pd.to_datetime("02/01/2005")]  # Born 2005
        })
        
        result_df = repair_sa_id_using_dob(df, "IdNo", "Dob")
        
        # Should be padded with "0" for 2001-2009 birth years
        assert result_df.loc[0, "IdNo"] == "05026128186"
    ...
    def test_no_repair_12_digit_id_not_born_2001_2009(self):
        ...
        df = pd.DataFrame({
            ...
            "Dob": [pd.to_datetime("02/01/2010")]  # Born 2010, not 2001-2009
        })
    ...
    def test_yymmdd_mismatch_no_repair(self):
        ...
        df = pd.DataFrame({
            "IdNo": [original_id],  # 11 digits
            "Dob": [pd.to_datetime("01/02/2001")]  # Different YYMMDD
        })
    ...
    def test_mixed_scenarios(self):
        ...
        df = pd.DataFrame({
            "IdNo": [
                ...
                "1026128186",  # 12 digits, born 2001 - should be repaired
                "7612026128186",  # 13 digits - no repair needed
                "01026128186",  # 11 digits, born 1999 - no repair
                "1026128186"   # 12 digits, born 2010 - no repair
            ],
            "Dob": [
                pd.to_datetime("02/01/2000"),  # 2000 - repair 11-digit
                pd.to_datetime("02/01/2001"),  # 2001 - repair 12-digit
                pd.to_datetime("02/12/1976"),  # 1976 - no repair needed
                pd.to_datetime("02/01/1999"),  # 1999 - no repair for 11-digit
                pd.to_datetime("02/01/2010")   # 2010 - no repair for 12-digit
            ]
        })
        ...
        assert result_df.loc[1, "IdNo"] == "01026128186"   # Repaired: 12-digit + 2001
        ...
        assert result_df.loc[4, "IdNo"] == "1026128186"    # Unchanged: 12-digit + 2010

    def test_string_dob_format(self):
        """Test handling of string DOB format."""
        df = pd.DataFrame({
            "IdNo": ["01026128186"],  # 11 digits
            "Dob": ["02/01/2000"]  # String date format
        })
        
        result_df = repair_sa_id_using_dob(df, "IdNo", "Dob")
        
        # Should be repaired (string DOB should be parsed)
        assert result_df.loc[0, "IdNo"] == "0001026128186"
    ...
    def test_none_values(self):
        ...
        df = pd.DataFrame({
            ...
            "Dob": [pd.to_datetime("02/01/2000"), None, pd.to_datetime("02/01/2000")]
        })
